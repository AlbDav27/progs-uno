...
/////////////////////Este es el bucle que se va a repetir para estar enviando y recibiendo constantemente información
	while (1){	
		...
		if (cp==pbr)
		{
			...
			//strcpy(com, "curl -X GET \"https://my.rayven.io:8082/api/customTableData?ctid=10&email=ebike@rayven.io&pw=qwerty\" >/home/alberto/Documents/r");
			...
			cmm=0;
			crchr=0;
			//fp = fopen ("/home/alberto/Documents/r","r");
			fseek(fp, 0L, SEEK_END);
			lf=ftell(fp);
			printf("\n Longitud del archivo : %i \n", lf);
			rewind(fp);
			while(((caracter = fgetc(fp)) != EOF)&& n<9000)
			{		
				re[n]=caracter;		
				//caracter = fgetc(fp);
				printf("%c",caracter);
